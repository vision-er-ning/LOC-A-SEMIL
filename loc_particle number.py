...
y_SEMIL = (0.9696, 0.62, 0.45, 0.432, 0.427, 0.428)

y_compute = (0.27, 0.36, 0.43, 0.49, 0.72, 0.89)
#x_particle = (100, 300, 500, 1000, 5000, 10000)
x_particle = ["100", "300", "500", "1000", "5000", "10000"]
...
rects1 = ax.bar(index, y_SEMIL, bar_width,
                color='r',
                error_kw=error_config,
                label='VISEL')

# rects2 = ax.bar(index + bar_width, y_SEMIL_without_semantic_map, bar_width,
#                 color='green',
#                 error_kw=error_config,
#                 label='SEMIL_Floor Map')
#
# rects3 = ax.bar(index + bar_width + bar_width, y_SEMIL, bar_width,
#                 color='r',
#                 error_kw=error_config,
#                 label='SEMIL')
...
ax.set_xticklabels(['100', '300', '500', '1000', '5000', '10000'])  #X轴横坐标
ax.legend()


plt.xlabel('Particle Number',size = 18)#横坐标命名
plt.ylabel('Mean Localization Error (m)',size = 18)
...
plt.grid()  #生成网格
plt.legend(loc='upper right',fontsize=10.5)#标签显示位置和大小   upper, lower
plt.subplots_adjust(top=0.991, bottom=0.11, right=0.996, left=0.096, hspace=0, wspace=0)
plt.savefig("localization_particle number.eps", dpi = 500)
plt.show()
